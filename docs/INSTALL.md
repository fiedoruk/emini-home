...
This guide installs emini Home 0.4.0 on a **ZECTRIX NOTE4C Devkit** from a
...
- From the [v0.4.0 release](https://github.com/fiedoruk/emini-home/releases/tag/v0.4.0):
  `emini-home-0.4.0-note4c.bin`, `emini-home-0.4.0-partition-table.bin`,
  `emini-home-0.4.0-sdkconfig.txt` and `SHA256SUMS`.
- A copy of this repository at tag `v0.4.0`, for `tools/preflight.py`.
  Download **Source code (zip)** from the same release and unpack it outside
  your installation folder. It unpacks into a folder named `emini-home-0.4.0`.
...
`certutil -hashfile emini-home-0.4.0-note4c.bin SHA256` and
`certutil -hashfile emini-home-0.4.0-partition-table.bin SHA256`, and compare
...
```sh
python3 path/to/emini-home-0.4.0/tools/preflight.py note4c-backup-a.bin note4c-backup-b.bin emini-home-0.4.0-partition-table.bin emini-home-0.4.0-note4c.bin
```
...
`Application file` line must say `0.4.0`. After `READY` it names the two files
...
```sh
esptool --chip esp32s3 -p PORT -b 460800 --after no-reset write-flash --flash-mode keep --flash-size keep --flash-freq keep 0x8000 emini-home-0.4.0-partition-table.bin 0x20000 emini-home-0.4.0-note4c.bin
```
...
```sh
esptool --chip esp32s3 -p PORT --after no-reset verify-flash 0x8000 emini-home-0.4.0-partition-table.bin 0x20000 emini-home-0.4.0-note4c.bin
```
